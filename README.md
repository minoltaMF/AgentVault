...
当前版本为 `0.1.0-alpha.5` internal alpha draft candidate；范围、平台状态和未覆盖能力见 [发布说明](docs/releases/0.1.0-alpha.5.md)。
...
AgentVault 目前没有公开稳定包。`0.1.0-alpha.5` 由版本标签触发 CI 生成内部 draft 制品；应从 AgentVault 仓库取得与目标 tag 对应且通过 CI 的制品，不能把 [cc-sessions Releases](https://github.com/ccpopy/cc-sessions/releases/latest) 中的上游程序当作 AgentVault。下列制品名称因兼容性而保留，本轮不迁移包名、binary 名或安装标识。
