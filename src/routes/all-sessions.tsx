...
export default function AllSessionsRoute() {
  ...
  return <Workbench key={JSON.stringify([settings.codex_dir, settings.claude_dir, settings.qoder_dir, settings.workbuddy_dir, settings.grok_dir, settings.pi_dir, settings.dsh_dir, settings.hermes_dir, settings.opencode_dir, settings.zcode_dir])} codexRoot={settings.codex_dir} claudeRoot={settings.claude_dir} qoderRoot={settings.qoder_dir ?? ""} workbuddyRoot={settings.workbuddy_dir ?? ""} grokRoot={settings.grok_dir ?? ""} piRoot={settings.pi_dir ?? ""} dshRoot={settings.dsh_dir ?? ""} hermesRoot={settings.hermes_dir ?? ""} opencodeRoot={settings.opencode_dir ?? ""} zcodeRoot={settings.zcode_dir ?? ""} />;
}

function Workbench({ codexRoot, claudeRoot, qoderRoot, workbuddyRoot, grokRoot, piRoot, dshRoot, hermesRoot, opencodeRoot, zcodeRoot }: { codexRoot: string; claudeRoot: string; qoderRoot: string; workbuddyRoot: string; grokRoot: string; piRoot: string; dshRoot: string; hermesRoot: string; opencodeRoot: string; zcodeRoot: string }) {
  ...
  const [savedView] = useState(() => workbenchCache.view(codexRoot, claudeRoot, qoderRoot, workbuddyRoot, grokRoot, piRoot, dshRoot, hermesRoot, opencodeRoot, zcodeRoot));
  ...
  const provider = ["codex", "claude", "qoder", "workbuddy", "grok", "pi", "dsh", "hermes", "opencode", "zcode"].includes(params.get("provider") ?? "") ? params.get("provider")! : "";
  const project = params.get("project") ?? "";
  const archive = ["active", "archived"].includes(params.get("archive") ?? "") ? params.get("archive")! : "";
  const all = useMemo(() => [...codex.sessions, ...claude.sessions, ...qoder.sessions, ...workbuddy.sessions, ...grok.sessions, ...pi.sessions, ...dsh.sessions, ...hermes.sessions, ...opencode.sessions, ...zcode.sessions], [codex.sessions, claude.sessions, qoder.sessions, workbuddy.sessions, grok.sessions, pi.sessions, dsh.sessions, hermes.sessions, opencode.sessions, zcode.sessions]);
  ...
  const contentScopeKey = JSON.stringify([codexRoot, claudeRoot, qoderRoot, workbuddyRoot, grokRoot, piRoot, dshRoot, hermesRoot, opencodeRoot, zcodeRoot, query, provider, project, archive, contentScopes]);
  ...
  return <>
    ...
    <ScrollArea className="min-h-0 flex-1" viewportRef={viewport}>
      <div className="space-y-5 p-4 md:p-6">
        <p className="text-sm text-muted-foreground">在一处查找 Codex、Claude、Qoder CLI、WorkBuddy、Grok Build CLI、Pi、DeepSeek Harness、Hermes Agent、OpenCode 与 ZCode 会话。选择读取来源后开始；预览为只读。</p>
        ...
        <div className="grid min-w-0 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="min-w-0 space-y-1 text-xs">来源<select aria-label="来源" className={selectClass} value={provider} onChange={(e) => filter("provider", e.target.value)}><option value="">所有来源</option><option value="codex">Codex</option><option value="claude">Claude</option><option value="qoder">Qoder CLI</option><option value="workbuddy">WorkBuddy</option><option value="grok">Grok Build CLI</option><option value="pi">Pi</option><option value="dsh">DeepSeek Harness</option><option value="hermes">Hermes Agent</option><option value="opencode">OpenCode</option><option value="zcode">ZCode</option></select></label>
    ...
    <ContentSearchDialog key={contentScopeKey} open={contentSearchOpen} onOpenChange={setContentSearchOpen} provider="codex" codexDir={codexRoot} claudeDir={claudeRoot} qoderDir={qoderRoot} workbuddyDir={workbuddyRoot} grokDir={grokRoot} piDir={piRoot} dshDir={dshRoot} hermesDir={hermesRoot} zcodeDir={zcodeRoot} opencodeDir={opencodeRoot} cursorDir="" showSubagentSessions={false} showArchivedSessions={archive === "archived"} rolloutPaths={[]} workbenchScopes={contentScopes} retained={retainedContent} onOpenResult={(session, match, text) => { setPreviewJump({ eventIndex: match.event_index, eventOffset: match.event_offset, query: text }); setPreview(session); setPreviewOpen(true); }} />
