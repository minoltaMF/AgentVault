...
export function useWorkbenchSource(provider: WorkbenchProvider, root: string, codexRoot: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const started = await api.startWorkbenchScan(provider, codexRoot, provider === "claude" ? root : "", provider === "qoder" ? root : undefined, provider === "workbuddy" ? root : undefined, provider === "grok" ? root : undefined, provider === "pi" ? root : undefined, provider === "dsh" ? root : undefined, provider === "hermes" ? root : undefined, provider === "opencode" ? root : undefined, provider === "zcode" ? root : undefined);
