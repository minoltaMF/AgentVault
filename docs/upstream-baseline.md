...
`crates/registry` 已提供 schema v2 canonical registry 与派生 FTS5 索引，但本轮不选择默认文件位置，也不接入 Tauri/CLI，因此不会创建或迁移任何实际应用数据。当前仍没有 AgentVault Vault 数据库。
...
- Provider SDK、canonical registry 和新搜索 API 尚未接入旧 Tauri/CLI 查询路径；当前没有搜索过滤/排序、不可变 Vault、Recovery Capsule 或新的默认数据目录。
...
- 下一步只增加 native resume plan 与 cwd mapping，不同时实现 Vault、Recovery Capsule 或原生 Session 写回。
