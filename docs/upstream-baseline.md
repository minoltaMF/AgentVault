...
当前代码继承 cc-sessions 的完整功能，尚未实现 AgentVault 开发方案中的新架构：
...
当前没有独立的 AgentVault Vault 数据库、canonical registry 或 FTS 数据库。
...
- 当前没有 Provider SDK、不可变 Vault、统一 canonical model、FTS 或 Recovery Capsule。
...
- 下一步只建立 Cargo workspace 和 `vault-io` crate，先抽取可复用的安全 I/O 边界，不改变业务行为。
- Provider SDK、Pi/canonical model、Vault、FTS 和 Recovery Capsule 必须分别进入后续提交，不能混入基线整理。
